...
class HubESN(ESNBase):
    ...
    def _generate_distant_constraints(self):
        ...
        # x = np.random.uniform(-1, 1, self.n_size)
        # y = np.random.uniform(-1, 1, self.n_size)
        # z = np.random.uniform(-1, 1, self.n_size)
    ...
    def _generate_sequence_contrainsts(self):
        ...
        return (seq_mat - np.min(seq_mat)) / (np.max(seq_mat) - np.min(seq_mat))
