...
class MNIST1():
    ...
    *** Part of the code is from https://blog.csdn.net/itnerd/article/details/109230929 
    ...
    def _init_task(self):
        ...
        # repeat each image 3 times
        self.X_train = np.repeat(self.X_train, 3, axis=0)
        self.X_test = np.repeat(self.X_test, 3, axis=0)
        self.y_train = np.repeat(self.y_train, 3, axis=0)
        self.y_test = np.repeat(self.y_test, 3, axis=0)
